    ...
    <div class="box-body">
        ...
        <div class="row" id="mostrarPorcentaje" style="display:none;">
    	  ...
    	  <div class="box-footer">
            ...
            <button type="button" id="btncarguedos" class="btn btn-primary">Cargar Subrogación</button>
      	</div><!-- /.box-footer -->
  	</div><!-- /.box -->
</div>
  	...
  	<div class="box" id="cargueunoaunoSki" style="display:none;">
        ...
        <div class="box-body">
                ...
                <div class="form-group">
                    <label for="inputEmail3" class="col-sm-2 control-label">No. liquidación / Contrato</label>
                    <div class="col-sm-10">
                ...
                <div class="form-group">
                    <label for="inputEmail3" class="col-sm-5 control-label">Fecha envío memorial de subrogación corregido</label>
                    <div class="col-sm-7">
                        <div class="input-group">
                            <div class="input-group-addon">
                               <i class="fa fa-calendar"></i>
                            </div>
                             <input type="text" class="form-control datemask" name="fechas2" id="fechas2"  placeholder= "dd/mm/yyyy">
                        </div><!-- /.input group -->
                    </div>
                </div><!-- /.form group -->
        </div><!-- /.box -->   
    	  <div class="box-footer">
            ...
            <button type="button" id="botonSolo" class="btn btn-primary">Cargar Subrogación</button>
      	</div><!-- /.box-footer -->
        </form> 
    </div>
          ...
          if(validador == 0){
              alertify.confirm("¿Esta seguro de esta operación?", function (e) {
                if (e) {
                    ...
                    $.ajax({
                        url: '<?php echo base_url(); ?>conceptos/guardarUnoaUno',  
                        type: 'POST',
                        data: { filtro :  $("#cmbFiltros2").val(), 
                                contrato :  $("#txtnumeroSap").val(), 
                                fechadevolucion: $("#fechas").val(), 
                                fechaenvio : $("#fechas2").val() },
              ...
              alertify.confirm("¿Esta seguro de esta operación?", function (e) {
                  if (e) {
                    ...
                    $.ajax({
                        ...
                        url: '<?php echo base_url(); ?>conceptos/cargar_subrogacionesMaxiba',  
                        type: 'POST',
                        data: formData,
                        cache: false,
                        contentType: false,
                        ...
                        success: function(data){
                             ...
                             alertify.success("La cantidad  de registros son "+ data.total +", asignados "+ data.acertados + ", " + data.noExisten + " no se pudieron asignar porque no existen en la base de datos ");
                              otherForm[0].reset();
