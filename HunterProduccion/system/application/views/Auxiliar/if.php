				...
				<div class="col-md-4">
					<form id="FrmAbogados" method="post">
						...
						<div class="form-group">
							<Label>Nombre IF</Label>
							<input type="text" class="form-control" placeholder="Nombre IF"  id="Ifses" name="Ifses" required disabled>
						</div>
					</form>
					...
					<div class="box">
						...
						<div class="box-body table-responsive no-padding">
							<table class="table table-hover" id="tblAbogados">
								<thead>
									<tr>
										<th style="text-align:center;">Nombre IF</th>
									</tr>
								</thead>
								<tbody>
									<?php
										foreach ($ids as $key) {
											echo '<tr style="cursor:pointer;">
													<td class="tdMinimo" onclick=" getdatos('.$key->G730_ConsInte__b.');">'.utf8_encode($key->G730_C17126).'</td>
...
<script type="text/javascript">
		...
		$("#delete").click(function(){
			alertify.confirm("¿Ésta seguro que desea eliminar el registro?", function (e) {
		    	if (e) {
	               	...
	               	$.ajax({
		            	url: '<?php echo base_url();?>auxiliar/eliminarIf',  
		            	type: 'POST',
			...
			if(form.valid()){
				alertify.confirm("¿Ésta seguro de esta operación?", function (e) {
	    			if (e) {
			          	...
			          	$.ajax({
			            	url: '<?php echo base_url();?>auxiliar/guardarIf',  
			            	type: 'POST',
				            data: formData,
			            	cache: false,
			            	contentType: false,
	...
	function getdatos(varid){
		...
		$.getJSON('<?php echo base_url();?>auxiliar/getDatosIf/'+varid, {format: "json"}, function(data) { 
