...
class Reportes extends CI_Controller {
        ...
        if($this->session->userdata('login_ok')){
            if( $frg != NULL){
                ...
                if($frg != 0){
                    ...
                    $this->db->where('G729_ConsInte__b', $frg);
                    $frgNOmbresaqui = $this->db->get('G729');
    ...
    function buscar_gestion_extrajudicial_mensual(){
        ...
        $fechaGrantia = date ( 'Y-m-d H:i:s' , $fechamesGarantia );
        //echo 'Fecha Para Garantia => '.$fechaGrantia.'</br>';

        $fechamesGarantiaFinal = strtotime ( '-1 minute' , strtotime ( $nuevafecha ) ) ;
        $fechaGrantiaFinal = date ( 'Y-m-d H:i:s' , $fechamesGarantiaFinal );
...
**/
    ...
    function getSubrogaciones_efectivas(){
        ...
        if( $frg != NULL && $frg != '' && $frg != 0){
            ...
            foreach ($frgs as $key2) {
                $this->db->select('COUNT(*) as cantidad');
                $this->db->from('Tabla_base_medicion_subrogaciones');
                $this->db->where('Sub_Frg', $key2->G729_ConsInte__b);
                $totalBaseMedicion = $this->db->get();

                $gestion = $this->Reportes_Model->getSUbrogaciones_Efectivas($key2->G729_ConsInte__b, $fechaInicial, $fechaFinal);
                $total = 0;
                /*foreach ($gestion as $key) {
                    $total++;
                }*/
                ...
                $gestion_deglosado = $this->Reportes_Model->getReportessubrogacionesEfectivas_deglosado($key2->G729_ConsInte__b, $fechaInicial, $fechaFinal);
                $viejo = 0;
                $nuevo = 0;
                foreach ($gestion_deglosado as $key) {
                    $nuevo = $key->contrato;
    ...
    function getSubrogaciones_efectivas_exportar($frg = NULL, $fechaInicial, $fechaFinal){
        ...
        if( $frg != NULL && $frg != '' && $frg != 0){
            ...
            $gestion = $this->Reportes_Model->getSUbrogaciones_Efectivas($frg, $fechaInicial, $fechaFinal.' 23:59:00');
            $gestion_deglosado = $this->Reportes_Model->getReportessubrogacionesEfectivas_deglosado($frg, $fechaInicial, $fechaFinal.' 23:59:00');
            $frgss = $this->db->get_where('G729',array('G729_ConsInte__b' => $frg));
            $this->db->select('par_meta, par_tiempo_asignacion, cant_obligaciones');
            $this->db->from('Parametros_reportes');
            $this->db->join('Reportes', 'par_rep_id = rep_id');
            ...
            foreach ($frgs as $key2) {
               ...
               /* foreach ($gestion as $key) {
                ...
                $gestion_deglosado = $this->Reportes_Model->getReportessubrogacionesEfectivas_deglosado($key2->G729_ConsInte__b, $fechaInicial, $fechaFinal.' 23:59:00');
                $nuevo = 0;
                $viejo = 0;
                foreach ($gestion_deglosado as $key) {
                    $nuevo = $key->contrato ;
        ...
        if($this->session->userdata('login_ok')){
            ...
            if( $frg != NULL && $frg != '' && $frg != 0){
                ...
                foreach ($gestion_deglosado as $key) {
                    ...
                    if($nuevo != $viejo){
                        $totalBaseMedicion +=1;
                        $gestion = $this->Reportes_Model->tieneGestionJudicial($key->G719_ConsInte__b, $fechaInicial, $fechaFinal);
                        if($gestion > 0){
                            $cumplimiento++;
                        }else{
                            $Nocumplen++;
                ...
                foreach ($frgs as $key2) {
                    ...
                    $gestion_deglosado = $this->Reportes_Model->getBaseGEstionesJudiciales_deglosado($key2->G729_ConsInte__b);
        ...
        if($this->session->userdata('login_ok')){
            ...
            if( $frg != NULL && $frg != '' && $frg != 0){
                ...
                foreach ($frgs as $key2) {
                    $cumplimiento = 0;
                    $Nocumplen = 0;
                    $gestion = $this->Reportes_Model->getBaseGEstionesJudiciales($key2->G729_ConsInte__b);
                    foreach ($gestion as $key) {
                        $totalBaseMedicion +=1;
                        $gestione = $this->Reportes_Model->tieneGestionJudicial($key->G719_ConsInte__b, $fechaInicial, $fechaFinal.' 23:59:00');
                        if($gestione >= 1){
                            $cumplimiento++;
                        }else{
                            $Nocumplen++;
