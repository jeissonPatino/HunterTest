...
class Cartera_fng extends CI_Controller {
    ...
    public function busquedaAvanzada(){
        if($this->session->userdata('login_ok')){
            ...
            $datos = array('clientes' => json_encode($data), 'filtros' => $filtro);
			$datosFooter = array('ul'=> 'ULcartera' , 'li' => 'LIjudicial');
    ...
    function eventoBotonEtapa(){
        if($this->session->userdata('login_ok')){
            ...
            foreach ($datosWizard as $key) {
                ...
                $estaGaver .= '<label>';
                $estaGaver .= '<input type="radio" name="optionsRadios" id="optionsRadios1" value="'. $key->G724_ConsInte__b .'">';
     ...
     function eventoBotonIlocalizado(){
        if($this->session->userdata('login_ok')){
            ...
            foreach ($datosWizard as $key) {
                $estaGaver .= '<div class="radio">';
                $estaGaver .= '<label>';
                $estaGaver .= '<input type="radio" name="localizadoSeleccionado" id="optionsRadios1" value="'. $key->gestion .'">';
    ...
    function getSubgestiones(){
        if($this->session->userdata('login_ok')){
            ...
            foreach ($datosWizard as $key) {
                $estaGaver .= '<div class="radio">';
                $estaGaver .= '<label>';
                $estaGaver .= '<input type="radio" name="cafeSeleccionado" id="optionsRadios1" value="'. $key->id .'">';
                $estaGaver .=  utf8_encode($key->enunciado);
                $estaGaver .= '</label>';
                $estaGaver .= '</div>';
            }
    ...
    function getSubgestionesTotal(){
        if($this->session->userdata('login_ok')){
            ...
            foreach ($datosWizard as $key) {
                $estaGaver .= '<div class="radio">';
                $estaGaver .= '<label>';
                $estaGaver .= '<input type="radio" name="cafeSeleccionadoTotal" id="optionsRadios1" value="'. $key->id .'">';
                $estaGaver .=  utf8_encode($key->enunciado);
                $estaGaver .= '</label>';
                $estaGaver .= '</div>';
            }
            ...
            if(!is_null($query->row()->liquidacion)){
                ...
                foreach ($resultado as $key2) {
                    ...
                    foreach ($datosObligacion as $key) {
                        ...
                        $array[$i]['fecha'] = "<span style='display: none;'>".$fecha3."</span>".$fecha2;
            ...
            if(!is_null($query->row()->liquidacion)){
                ...
                foreach ($resultado as $key2) {
                    ...
                    foreach ($datosObligacion as $key) {
                        ...
                        $datosarray[$i]['fecha'] = "<span style='display: none;'>".$fecha[0].$fecha[1].$fecha[2]."</span>".$fecha[2]."/". $fecha[1]."/". $fecha[0];
        ...
        if($this->session->userdata('login_ok')){
            ...
            foreach ($datosObligacion as $key) {
                ...
                if(!is_null($key->Fecha_aprovacion_soporte)){
                    $Fecha_aprovacion_soporte = explode(" ", $key->Fecha_aprovacion_soporte)[0];
                    $Fecha_aprovacion_soporte = explode("-", $Fecha_aprovacion_soporte);
                    $Fecha_aprovacion_soporte = $Fecha_aprovacion_soporte[2]."/".$Fecha_aprovacion_soporte[1]."/".$Fecha_aprovacion_soporte[0];
                }
                        ...
                        <div id="collapseCISA" class="panel-collapse collapse ">
                            <div class="box-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">'.$key->VALOR_FACTURADO_SOPORTES_CISA.' </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3" >'.$Fecha_aprovacion_soporte.' </div>
                                </div>
                            </div>
                        </div>';
    ...
    function getinformacionJudicial($Contrato){
        if($this->session->userdata('login_ok')){
            ...
            foreach ($datosObligacion as $key ) {
                ...
                $this->db->select('NumeroContrato as txtFechaTramite');
                $this->db->from('G735');
    ...
    function getGarantias($Contrato){
        if($this->session->userdata('login_ok')){
            ...
            if($liquidacion == 0){
                ...
                $datosObligacion = $this->Wizard_Model->getGarantiasxLiquidacion($liquidacion);
            }
        ...
        if($this->session->userdata('login_ok')){
            ...
            foreach ($contratos as $contrato) {
                ...
                $datos = array( 'FechaEjecucion' => $fechaIngreso,
                            ...
                            'NumeroContrato' => $contrato->No_CONTRATO,
                            'Ejecutor' => $user,
                            'IdCliente' => $idUduario,
    ...
    function guardardatosWizard(){
        if($this->session->userdata('login_ok')){
            ...
            $datos = array( 'Actuacion' => $_POST['actuacion'],
                            ...
                            'NumeroContrato' => $_POST['txtFechaTramite'],
                            'G735_C17140' => $user,
