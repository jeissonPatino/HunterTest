...
class Auxiliar extends CI_Controller {
    ...
    function carguemasAbogadosSuperFNG(){
        if($this->session->userdata('login_ok')){
            ...
            /* Requerimiento 11 Formatear Fecha ingresada Daniel Salgado */
                ...
                if ( $index > 0 ){
                    ...
                    if($value['A'] != ""){
                        if($this->Wizard_Model->validarSAP($value['A'])){
                            ...
                            $fechaAbogado = $value['B'];
                            if(strpos($value['B'], "/")){
                                $fechaAsignacion = explode("/", $value['B']);
                                if (strlen($fechaAsignacion[0]) == 2) $fechaAsignacion[2] = '20'.$fechaAsignacion[0];
                                $fechaAbogado = $fechaAsignacion[0]."-".$fechaAsignacion[1]."-".$fechaAsignacion[2];
                            }
                            $datos = array(
        ...
        if($this->session->userdata('login_ok')){
            ...
            foreach ($result2 as $key2) {
                ...
                if($idNuevo != $idViejo){
                    ...
                    $datosAdcicionales[$i]['tipo_identificacion'] = utf8_encode($key2->tipo_identificacion) ;
            ...
            foreach ($result as $key) {
                ...
                $datosAdcicionales[$i]['tipo_identificacion'] = utf8_encode($key->tipo_identificacion) ;
    ...
    function getJudicialesForDelete(){
        if($this->session->userdata('login_ok')){
            ...
            $this->load->view('Includes/header');
            $this->load->view('Includes/sidebar');
            $this->load->view('Auxiliar/judicial');
            $this->load->view('Includes/footer', $datosFooter);
        }else{
            $this->load->view('Login/login');
        }
        ...
        //esta es para eliminar las gestiones judiciales mal echas
    function eliminarGestionExtrajudiciales(){
        if($this->session->userdata('login_ok')){
            ini_set('memory_limit', '1024M');
    ...
    function eliminarGestionExtraJudicial(){
        ...
        $llenarTablaLog = $this->reportes_Model->LlenarTablaLogeliminacionLiquidacionExtra($usuario,$id,$numLiquidacion);
            $ValEliminar = $this->Extrajudicial_Model->eliminarGestionExtraJudicial($_POST['IdEliminar']);
            if ($ValEliminar == 0){
